-- ShiftSync V2 seed for Supabase SQL editor
-- Run AFTER V1__schema.sql (safe to re-run: ON CONFLICT DO NOTHING)
-- Password for ALL seeded users: 1234
-- Full login list: docs/supabase/README.md
--
-- Deterministic UUIDs (valid for Postgres UUID columns):
--   site  a0000000-0000-4000-8000-000000000001
--   dept  b0000000-0000-4000-8000-00000000000N
--   cert  c0000000-0000-4000-8000-00000000000N
--   user  dN000000-0000-4000-8000-00000000000N

INSERT INTO sites (id, name) VALUES
('a0000000-0000-4000-8000-000000000001', 'Obuasi Mine')
ON CONFLICT (id) DO NOTHING;

-- departments (id, site_id, name)
INSERT INTO departments (id, site_id, name) VALUES
('b0000000-0000-4000-8000-000000000001', 'a0000000-0000-4000-8000-000000000001', 'Underground Zone 1'),
('b0000000-0000-4000-8000-000000000002', 'a0000000-0000-4000-8000-000000000001', 'Underground Zone 2'),
('b0000000-0000-4000-8000-000000000003', 'a0000000-0000-4000-8000-000000000001', 'Surface Processing')
ON CONFLICT (id) DO NOTHING;

UPDATE departments SET mine_zone = 'UG-Z1' WHERE id = 'b0000000-0000-4000-8000-000000000001';
UPDATE departments SET mine_zone = 'UG-Z2' WHERE id = 'b0000000-0000-4000-8000-000000000002';
UPDATE departments SET mine_zone = 'SURF'  WHERE id = 'b0000000-0000-4000-8000-000000000003';

-- certifications (id, site_id, name, description, expiry_days)
INSERT INTO certifications (id, site_id, name, description, expiry_days) VALUES
('c0000000-0000-4000-8000-000000000001', 'a0000000-0000-4000-8000-000000000001', 'Underground Blasting', 'Blasting operations safety', 365),
('c0000000-0000-4000-8000-000000000002', 'a0000000-0000-4000-8000-000000000001', 'First Aid L2', 'Basic life support', 730)
ON CONFLICT (id) DO NOTHING;

-- Supervisors
INSERT INTO users (id, email, password_hash, display_name, role, department_id, employee_no) VALUES
('d1000000-0000-4000-8000-000000000001', 'kwame@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Kwame Mensah', 'SUPERVISOR', 'b0000000-0000-4000-8000-000000000001', 'SUP-100'),
('d1000000-0000-4000-8000-000000000002', 'akua@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Akua Osei', 'SUPERVISOR', 'b0000000-0000-4000-8000-000000000002', 'SUP-101'),
('d1000000-0000-4000-8000-000000000003', 'yaw@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Yaw Appiah', 'SUPERVISOR', 'b0000000-0000-4000-8000-000000000003', 'SUP-102')
ON CONFLICT (id) DO NOTHING;

UPDATE departments SET supervisor_id = 'd1000000-0000-4000-8000-000000000001' WHERE id = 'b0000000-0000-4000-8000-000000000001';
UPDATE departments SET supervisor_id = 'd1000000-0000-4000-8000-000000000002' WHERE id = 'b0000000-0000-4000-8000-000000000002';
UPDATE departments SET supervisor_id = 'd1000000-0000-4000-8000-000000000003' WHERE id = 'b0000000-0000-4000-8000-000000000003';

INSERT INTO users (id, email, password_hash, display_name, role, employee_no) VALUES
('d2000000-0000-4000-8000-000000000001', 'admin@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Afiya Baah', 'ADMIN', 'ADM-001'),
('d3000000-0000-4000-8000-000000000001', 'safety@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Kofi Owusu', 'SAFETY', 'SAF-001')
ON CONFLICT (id) DO NOTHING;

INSERT INTO users (id, email, password_hash, display_name, role, department_id, employee_no) VALUES
('d4000000-0000-4000-8000-000000000001', 'ama.boateng@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Ama Boateng', 'WORKER', 'b0000000-0000-4000-8000-000000000002', 'WRK-1000'),
('d4000000-0000-4000-8000-000000000002', 'kwadwo.asare@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Kwadwo Asare', 'WORKER', 'b0000000-0000-4000-8000-000000000003', 'WRK-1001'),
('d4000000-0000-4000-8000-000000000003', 'abena.pokua@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Abena Pokua', 'WORKER', 'b0000000-0000-4000-8000-000000000001', 'WRK-1002'),
('d4000000-0000-4000-8000-000000000004', 'kwabena.frimpong@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Kwabena Frimpong', 'WORKER', 'b0000000-0000-4000-8000-000000000002', 'WRK-1003'),
('d4000000-0000-4000-8000-000000000005', 'yaa.asantewaa@shiftsync.io', '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq', 'Yaa Asantewaa', 'WORKER', 'b0000000-0000-4000-8000-000000000003', 'WRK-1004')
ON CONFLICT (id) DO NOTHING;

-- Re-runnable: force PIN to 1234 for all seeded accounts (even if rows already existed)
UPDATE users
SET password_hash = '$2b$12$Z4lNP866zNB1o4FvIMmXxOZsi/PqA8Kq8D9jBwQ./F4b9s9Cm9OTq',
    updated_at = now()
WHERE id IN (
  'd1000000-0000-4000-8000-000000000001',
  'd1000000-0000-4000-8000-000000000002',
  'd1000000-0000-4000-8000-000000000003',
  'd2000000-0000-4000-8000-000000000001',
  'd3000000-0000-4000-8000-000000000001',
  'd4000000-0000-4000-8000-000000000001',
  'd4000000-0000-4000-8000-000000000002',
  'd4000000-0000-4000-8000-000000000003',
  'd4000000-0000-4000-8000-000000000004',
  'd4000000-0000-4000-8000-000000000005'
);

INSERT INTO user_certifications (user_id, cert_id, issued_at, expires_at) VALUES
('d4000000-0000-4000-8000-000000000003', 'c0000000-0000-4000-8000-000000000001', CURRENT_DATE - interval '100 days', CURRENT_DATE + interval '265 days')
ON CONFLICT (user_id, cert_id) DO NOTHING;

INSERT INTO shifts (id, department_id, start_time, end_time, shift_type, required_workers, status, created_by, published_at) VALUES
('e0000000-0000-4000-8000-000000000001', 'b0000000-0000-4000-8000-000000000001', CURRENT_DATE + interval '6 hours', CURRENT_DATE + interval '18 hours', 'DAY', 5, 'PUBLISHED', 'd1000000-0000-4000-8000-000000000001', CURRENT_TIMESTAMP)
ON CONFLICT (id) DO NOTHING;

INSERT INTO shift_assignments (id, shift_id, user_id, status) VALUES
('e1000000-0000-4000-8000-000000000001', 'e0000000-0000-4000-8000-000000000001', 'd4000000-0000-4000-8000-000000000003', 'CONFIRMED')
ON CONFLICT (id) DO NOTHING;

INSERT INTO fatigue_scores (id, user_id, calculated_at, hours_worked_24h, hours_worked_7d, night_shifts_7d, consecutive_days, score, risk_level, model_version) VALUES
('e2000000-0000-4000-8000-000000000001', 'd4000000-0000-4000-8000-000000000001', CURRENT_TIMESTAMP, 12, 72, 4, 7, 85, 'CRITICAL', 'FAID-mock-v1'),
('e2000000-0000-4000-8000-000000000002', 'd4000000-0000-4000-8000-000000000002', CURRENT_TIMESTAMP, 12, 72, 4, 7, 85, 'CRITICAL', 'FAID-mock-v1'),
('e2000000-0000-4000-8000-000000000003', 'd4000000-0000-4000-8000-000000000003', CURRENT_TIMESTAMP, 8, 40, 0, 3, 65, 'WARNING', 'FAID-mock-v1')
ON CONFLICT (id) DO NOTHING;
